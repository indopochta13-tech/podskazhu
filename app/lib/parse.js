...
// Месяц без числа: «в сентябре», «к октябрю», «ближе к декабрю».
// Отдельный список, потому что падежи тут другие, а число не названо вовсе.
const MONTH_ALONE = [
  "январ(?:е|ю)", "феврал(?:е|ю)", "март(?:е|у)", "апрел(?:е|ю)", "ма(?:е|ю)",
  "июн(?:е|ю)", "июл(?:е|ю)", "август(?:е|у)", "сентябр(?:е|ю)", "октябр(?:е|ю)",
  "ноябр(?:е|ю)", "декабр(?:е|ю)",
];
...
const DAY_PARTS = [
  { pattern: "(?:с\\s+|со\\s+)?утр(?:ом|а|ечком|ечка|еца)|к\\s+утру", hour: 9 },
...
const VAGUE_TIMES = [
  ...
  { pattern: "за\\s+завтраком|после\\s+завтрака|к\\s+завтраку", hour: 9 },
  { pattern: "в\\s+обеденный\\s+перерыв|в\\s+перерыв", hour: 13 },
  ...
  { pattern: "за\\s+ужином|во\\s+время\\s+ужина|к\\s+ужину", hour: 19 },
...
const MEETING_RE = new RegExp(
  `${NBL}(?:встреч|встрет(?:иться|имся|ится|ились)|созвон|созвонимс|созвонит|митинг|совещани|собрани|планерк|интервью|прием|визит|конференц`
  ...
  // Обрывок без предлога: «врач в 15», «зубной завтра» — это всё равно приём.
  + `|врач${NB}|врача${NB}|зубной${NB}|стоматолог${NB}|терапевт${NB}`
  + `|к\\s+(?:врачу|зубному|стоматологу|нотариусу|терапевту|педиатру|мастеру|парикмахеру|бровист|космето`
  + `|окулисту|офтальмологу|лору${NB}|хирургу|гинекологу|урологу|дерматологу|эндокринологу|кардиологу|неврологу)`
  // «Записаться на …» — сама запись это встреча, что бы ни стояло дальше.
  + `|запис(?:аться|ываться|ать)\\s+на${NB}`
  + `|консультац|собеседовани|техосмотр|аттестац|защита\\s+диплом`
...
const BUY_RE = new RegExp(
  ...
  + `|продукт(?:ы|ов|ами|ам)${NB}|за\\s+продуктами${NB}`
  ...
  // Заказ припасов — покупка; заказ еды на вечер — разовое дело, а не список.
  + `|(?:заказать|закажи|заказ)\\s+(?:${W}+\\s+)?(?:воду${NB}|воды${NB}|продукт)`
...
const SPORT_RE = new RegExp(
  ...
  + `|зал${NB}|зале${NB}|на\\s+(?:тренировк|зал)${NB}|занятие\\s+(?:в\\s+зале|спортом)`
  + `|кача(?:ть|ю|ем)\\s+(?:ноги|ногу|грудь|спину|руки|руку|плечи|пресс|ягодиц)`
  + `|день\\s+(?:ног|груди|спины|рук|плеч|пресса)`
  + `|растяжк|потяну(?:ться|ся)|пресс${NB}(?!\\s*-?\\s*(?:релиз|конференц|секретар|служб|цент))`
  + `|планк[аиу]${NB}|отжимани|подтягивани`
  + `|трениру(?:ю|ем|ешь|й))`,
  "iu");
...
const CARE_RE = new RegExp(
  ...
  + `|очищен(?:ие|ия)|пенк${W}*${NB}|патч(?:и|ей)|умыван|умы(?:ться|ваться)`
...
const METERS_RE = new RegExp(
  ...
  // «Передать воду и свет» — про счётчики говорят так же часто, как «показания».
  + `|переда(?:ть|й|м)\\s+(?:${W}+\\s+){0,2}(?:воду${NB}|свет${NB}|газ${NB}|электр)`
...
const BILLS_RE = new RegExp(
  ...
  + `|(?:денег|деньги)\\s+на\\s+(?:телефон|счет|карт)`
  // Падежи и множественное: «штрафы», «налогам», «кредиты» — то же самое.
  // Раньше стояло ${NB}, и любая форма кроме именительного падала в дела.
  + `|ипотек|кредит(?:ы|а|у|ов|ам|ами|ах|е)?${NB}|налог(?:и|а|у|ов|ам|ами|ах|е)?${NB}`
  + `|штраф(?:ы|а|у|ов|ам|ами|ах|е)?${NB}|осаго${NB}|каско${NB}`
  + `|(?:перевести|перевод|скинуть|внести)\\s+(?:${W}+\\s+)?за\\s+(?:аренд|квартир|садик|учеб|кварт)`
  + `|за\\s+аренду${NB}|долг${W}*\\s+по${NB}`
...
const HEALTH_RE = new RegExp(
  ...
  + `|прививк|мазь${NB}|сироп|ингаля|давлени|анализ${W}*${NB}|температур`
  // Лекарства человек называет по действию, а не по названию на коробке.
  + `|обезболива|жаропонижа|противовирус|антигистамин|снотворн|слабительн`
  // Процедуры и обследования — та же полка, что анализы: важна регулярность.
  + `|флюорограф|рентген|узи${NB}|мрт${NB}|экг${NB}|кардиограм|капельниц|физиотерап`
  + `|(?:измерить|померить|мерить|померять)\\s+(?:давлени|температур)|курс\\s+лечен`
  // «Сдать кровь» — тот же анализ, только назван по материалу, а не по бумажке.
  + `|(?:сда(?:ть|й|ча)|сдал[аи]?)\\s+кровь${NB}|кровь\\s+из\\s+вены|анализ${W}*\\s+кров`
  + `|(?:выпить|принять|попить|пить|пропить)\\s+(?:${W}+\\s+)?(?:таблетк|лекарств|витамин|капл|сироп|антибиотик|обезболива|жаропонижа)`
  // «Пропить курс магния» — курс добавок живёт там же, где курс лечения.
  + `|(?:пропить|пропей|пропива)${W}*\\s+курс|курс${W}*\\s+(?:магни|железа|омег|цинк|кальци|витамин))`,
  "iu");
...
const SPLIT_VERBS = [
  ...
  "пода(?:ть|й)", "офорим(?:ить)", "отпра(?:вить)", "запол(?:нить|ни)",
].join("|");
...
/**
 * Праздники как узел календаря.
 *
 * «После праздников» без уточнения в России почти всегда значит «после новогодних»:
 * 1–8 января нерабочие, и дело откладывают на первый рабочий день после них.
 * Поэтому берём ближайшее 1 января и считаем от него, а не ищем ближайший
 * красный день в году — иначе фраза, сказанная в августе, уехала бы на 4 ноября,
 * чего человек не имел в виду.
 */
function nextNewYear(nowParts) {
  const thisYear = { year: nowParts.year, month: 0, day: 1 };
  return compareDates(thisYear, nowParts) > 0 ? thisYear : { year: nowParts.year + 1, month: 0, day: 1 };
}

function firstWorkdayAfter(date) {
  let out = date;
  while (weekdayOf(out) === 0 || weekdayOf(out) === 6) out = addDays(out, 1);
  return out;
}

const HOLIDAY_RULES = [
  // «перед Новым годом» — последние дни декабря, когда ещё можно успеть.
  { pattern: `(?:перед|до|к)\\s+новым\\s+годом|перед\\s+новый\\s+год`, at: ny => addDays(ny, -2) },
  { pattern: `(?:после|за)\\s+(?:новогодних\\s+)?праздник${W}*|после\\s+нового\\s+года`, at: ny => firstWorkdayAfter(addDays(ny, 8)) },
  { pattern: `(?:на|в)\\s+новогодни${W}*\\s+праздник${W}*|(?:на|в)\\s+каникул${W}*`, at: ny => addDays(ny, 1) },
  { pattern: `(?:на|в|под)\\s+новый\\s+год${NB}`, at: ny => ny },
];
...
// Половина, четверть и «без пяти» считают час одинаково, поэтому и уточнение
// половины суток разбирают одинаково. Раньше каждая форма делала это сама
// и все три забывали про «дня»: «а точнее в полтретьего» давало 02:30.
function applyClockMod(hour, minute, mod, nowParts, dayKnown, wakeUp) {
  if (/ночи/.test(mod)) return hour === 12 ? 0 : (hour >= 8 ? hour + 12 : hour);
  if (/вечера/.test(mod) && hour < 12) return hour + 12;
  if (/дня/.test(mod) && hour <= 5) return hour + 12;
  if (/утра/.test(mod)) return hour === 12 ? 0 : hour;
  if (!mod) return resolveBareHour(hour, minute, nowParts, dayKnown, wakeUp);
  return hour;
}
...
export function extractWhen(text, ctx, opts = {}) {
  ...
  const THROUGH_NUM = `\\d{1,3}|полчаса|полгода|полтора|полторы|пару|несколько|день${NB}|сутки${NB}|час${W}*|недел${W}*|месяц${W}*|(?:${NUM_WORD_ALT})${NB}`;
  ...
  if (through) {
    ...
    // Полгода считаем как шесть месяцев по тридцать дней — той же мерой,
    // что и «через месяц» выше, иначе два соседних оборота разошлись бы.
    else if (/^полгода$/.test(rawNum)) ms = 180 * 86400000;
    else {
  ...
  // «на днях» — не сегодня и не через неделю: ближайшие пару дней.
  // Без правила оборот уезжал в место («Днях заехать»).
  if (!date) {
    const m = firstMatch(low, mask, `${NBL}(?:на|в)\\s+дн(?:ях|яx)${NB}`);
    if (m) {
      date = addDays(nowParts, 2);
      markRange(mask, m.index, m.index + m[0].length);
    }
  }
  ...
  if (!date) {
    const m = firstMatch(low, mask, `${NBL}(?:ближе\\s+к\\s+|(?:на|в|во|к)\\s+)?(?:(эт(?:и|их)|ближайш${W}*|следующ${W}*|будущ${W}*)\\s+)?(?:выходны${W}*|вых(?:ах|и|ами)${NB})`);
    if (m) {
  ...
  const WEEKDAY_PREP = "(?:начиная\\s+)?(?:в|во|до|к|ко|на|с|со)";
  let weekShift = 0;
  {
  ...
  if (!date) {
    const m = firstMatch(low, mask, `${NBL}(?:в\\s+)?(?:(начал|середин|конц|конец)${W}*|(перв${W}*\\s+числ${W}*))\\s+(?:(следующ${W}*|этот|этом|это[йм])\\s+)?(недел${W}*|месяц${W}*|квартал${W}*|год${W}*|${MONTH_PATTERNS.join("|")})${NB}`);
    if (m) {
      const part = m[2] ? "начал" : (m[1] === "конец" ? "конц" : m[1]);
  ...
  // Обрывок без предлога: «отчет пятница», «стрижка суббота», «бассейн четверг».
  // Последняя попытка по дню недели — все формы с предлогом уже разобраны выше,
  // так что сюда доходит только голое название дня.
  if (!date) {
    for (const wd of WEEKDAYS) {
      const m = firstMatch(low, mask, `${NBL}(?:${wd.pattern})${NB}`);
      if (m) {
        date = addDays(nowParts, weekdayDelta(nowParts.weekday, wd.idx, false) + weekShift);
        markRange(mask, m.index, m.index + m[0].length);
        break;
  ...
  if (!date) {
    const m = firstMatch(low, mask, `${NBL}числа\\s+(?:до\\s+)?${ORDINAL_DAY_PATTERN}${NB}`);
    if (m) {
      const day = ordinalDay(m[1], m[2]);
      if (day != null) {
        date = dateOnDayNumber(day, nowParts);
  ...
  // «перед Новым годом», «после праздников», «на новогодние каникулы».
  if (!date) {
    for (const rule of HOLIDAY_RULES) {
      const m = firstMatch(low, mask, `${NBL}(?:${rule.pattern})`);
      if (!m) continue;
      date = rule.at(nextNewYear(nowParts));
      markRange(mask, m.index, m.index + m[0].length);
      break;
    }
  }

  // «в сентябре отпуск», «к декабрю подготовить» — числа нет, значит начало месяца.
  if (!date) {
    for (let i = 0; i < MONTH_ALONE.length; i += 1) {
      const m = firstMatch(low, mask, `${NBL}(?:в|во|к|ко|на|ближе\\s+к)\\s+(?:${MONTH_ALONE[i]})${NB}`);
      if (!m) continue;
      const year = i < nowParts.month ? nowParts.year + 1 : nowParts.year;
      // Текущий месяц: первое число уже прошло, значит речь о следующем годе.
      date = (i === nowParts.month && nowParts.day > 1)
        ? { year: year + 1, month: i, day: 1 }
        : { year, month: i, day: 1 };
      markRange(mask, m.index, m.index + m[0].length);
      break;
    }
  ...
  if (!time) {
    ...
    if (m) {
      ...
      if (ord != null) {
        ...
        hour = applyClockMod(hour, 30, mod, nowParts, dayKnown(), wakeUp);
  ...
  if (!time) {
    ...
    if (m) {
      ...
      if (ord != null) {
        let hour = (ord + 11) % 12 || 12;
        const mod = m[2] || partHint;
        hour = applyClockMod(hour, 15, mod, nowParts, dayKnown(), wakeUp);
  ...
  if (!time) {
    ...
    if (m) {
      ...
      if (minute != null && minute >= 1 && minute <= 59 && ord != null) {
        ...
        hour = applyClockMod(hour, minute, mod, nowParts, dayKnown(), wakeUp);
  ...
  // «садик 8 утра», «созвон 9 вечера» — предлога нет, но половина суток названа.
  // «дня» сюда намеренно не берём: «на 3 дня» — это длительность, а не три часа дня.
  if (!time) {
    const m = firstMatch(low, mask, `${NBL}(\\d{1,2})\\s*(?:час${W}*)?\\s+(утра|вечера|ночи)${NB}`);
    if (m) {
      const hour = Number(m[1]);
      if (hour >= 1 && hour <= 23) {
        time = { hour: resolveHour(hour, 0, m[2], nowParts, dayKnown(), wakeUp), minute: 0 };
        markRange(mask, m.index, m.index + m[0].length);
      }
    }
  }
  ...
  // Обрывок «митинг 11», «созвон 15» — число в самом конце и есть час.
  // Диапазон 6..23 отсекает счёт («хлеб 2», «яйца 10» — не время),
  // а требование конца фразы — количество перед предметом («купить 10 яиц»).
  if (!time && !timer) {
    const m = firstMatch(low, mask, `${NBL}(?<!(?:квартир[аыу]|дом|комнат[аы]|корпус|подъезд|этаж|кабинет|размер|номер|страниц[аы]|вагон|палат[аы])\\s)(\\d{1,2})\\s*$`);
    if (m) {
      const hour = Number(m[1]);
      const words = low.trim().split(/\s+/).filter(Boolean);
      if (hour >= 6 && hour <= 23 && words.length >= 2) {
        time = { hour: resolveHour(hour, 0, partHint, nowParts, dayKnown(), wakeUp), minute: 0 };
        markRange(mask, m.index, m.index + m[0].length);
      }
    }
  }
...
function splitCorrection(chunk) {
  ...
  const rawTail = chunk.slice(last.index + last[0].length).trim();
  const tail = dropLeadMarkers(rawTail);
  if (!head || !tail) return null;
  // Вводных подряд бывает несколько («хотя нет»), и решает смысл всей связки,
  // а не первого слова. Знаки убираем: маркер приходит вида «, тьфу,».
  const marker = normalize(`${last[0]} ${rawTail.slice(0, rawTail.length - tail.length)}`)
...
// Отменяющие вводные: после них сказанное раньше не «дополняется», а зачёркивается.
// «Лучше», «или давай» так не работают — они уточняют, а не отменяют.
const NEGATING_MARKER_RE = /(?:^|\s)(?:нет|тьфу|не\s+то|пардон|извини)(?:\s|$)/iu;

// «Купить молоко, хотя нет, кефир» — человек заменил предмет, а не завёл второе дело.
// Признак узкий нарочно: отменяющая вводная, во второй половине одно слово,
// и ни там, ни там нет ни времени, ни даты, ни места. Иначе разбираем как раньше.
function isObjectSwap(fix, head, tail, tailPlace) {
  if (!fix.marker || !NEGATING_MARKER_RE.test(` ${fix.marker} `)) return false;
  if (head.time || tail.time || tailPlace) return false;
  if ((head.date && !head.dateInferred) || (tail.date && !tail.dateInferred)) return false;
  const tailWords = normalize(fix.tail).split(/[^а-яa-z0-9]+/u).filter(Boolean);
  if (tailWords.length !== 1 || tailWords[0].length < 3) return false;
  return normalize(head.rest || fix.head).split(/\s+/).filter(Boolean).length >= 2;
}

// Меняем последнее слово головы на то, что назвали взамен.
function swapLastWord(text, replacement) {
  const words = String(text || "").trim().split(/\s+/).filter(Boolean);
  if (words.length < 2) return `${text} ${replacement}`.trim();
...
const GROCERY_RE = new RegExp(
  ...
  + `|подгузник|памперс|салфетк|стиральн|порошок|мыло|шампунь|зубн${W}*\\s+паст`
  // Расходники и еда, которые называют без глагола: «туалетная бумага», «мясо на ужин».
  + `|туалетн${W}*\\s+бумаг|батаре[еий]к|лампочк|губк[аиу]${NB}|щетк|кроссовк|кед[ыи]${NB}`
  + `|мяс[оа]${NB}|куриц|рыб[аыу]${NB}|полотенц|пакет(?:ы|ов)${NB}|носк[иов]`
...
const GROCERY_BLOCK_RE = new RegExp(
  ...
  + `|размороз(?:ить|ь)|постира(?:ть|й)|погла(?:дить|дь)|высуш(?:ить|и)|слома(?:лся|лась|лось)`
  + `|почин(?:ить|и)|помы(?:ть|й)|убра(?:ть)|вынес(?:ти|и)|разлож(?:ить|и)|порез(?:ать|ь))`,
  "iu",
);
// Место покупки, названное без глагола. Отдельно от BUY_RE, потому что вес другой.
const SHOP_PLACE_RE = new RegExp(
  `${NBL}(?:аптек[аиуе]${NB}|магазин${NB}|супермаркет${NB}|рынок${NB}|пятерочк|перекресток|ашан${NB}|вкусвилл|магнит${NB})`,
  "iu",
);
...
export function classifyKind(chunk) {
  ...
  // Совсем обрывок: названо одно место покупки и всё («аптека», «магазин»).
  // Стоит после глагола нарочно — «позвонить в аптеку» это дело, а не покупка.
  if (!hits.length && SHOP_PLACE_RE.test(text)) add("buy", 80);
...
/**
 * Слух микрофона: «читверг», «аплатить», «малако», «хлеп».
 *
 * Списком опечаток это не закрыть — их бесконечно много. Зато у них одна
 * природа: безударные о/а и е/и сливаются, звонкий на конце оглушается,
 * мягкий знак и удвоение теряются. Сводим слово к «глухому ключу», где все
 * эти различия стёрты, и сверяем со словарём слов, которые решают судьбу
 * записи: дни недели, месяцы, полки, глаголы действия.
 *
 * Словарь закрытый и намеренно узкий. Правим только то, что не является
 * словом из самого словаря, только при единственном совпадении ключа
 * и только для слов от пяти букв: на коротких ключи сталкиваются
 * («пять» и «пить» дают один ключ, а перепутать их нельзя).
 */
const STT_VOCAB = [
  // дни недели — от них зависит дата
  "понедельник", "понедельника", "понедельнику", "вторник", "вторника", "вторнику",
  "среду", "среды", "среде", "четверг", "четверга", "четвергу",
  "пятницу", "пятница", "пятницы", "пятнице", "субботу", "суббота", "субботы", "субботе",
  "воскресенье", "воскресенья", "воскресенью",
  // месяцы
  "января", "январь", "февраля", "февраль", "марта", "апреля", "апрель",
  "августа", "август", "сентября", "сентябрь", "октября", "октябрь",
  "ноября", "ноябрь", "декабря", "декабрь",
  // Предложный и дательный: без них ключ «сентябре» совпадал с «сентября»,
  // и правильная форма молча переписывалась в неправильную.
  "январе", "январю", "феврале", "февралю", "марте", "марту", "апреле", "апрелю",
  "мае", "маю", "июне", "июню", "июле", "июлю", "августе", "августу",
  "сентябре", "сентябрю", "октябре", "октябрю", "ноябре", "ноябрю",
  "декабре", "декабрю",
  // слова полок — от них зависит, куда ляжет запись
  "оплатить", "заплатить", "продукты", "молоко", "тренировка", "тренировку",
  "совещание", "встреча", "встречу", "встречи", "таблетки", "таблетку", "витамины",
  "заметка", "заметку", "рождения", "передать", "показания", "будильник",
  "анализы", "квартиру", "интернет", "магазин", "аптеку", "аптека", "подъем",
  // существительные, по которым выбирается полка. Глагол может уцелеть,
  // а полку задаёт именно предмет: «аплатить каммуналку» без этого слова
  // оставалось делом, а не платежом.
  "коммуналка", "коммуналку", "коммуналки", "квартплата", "квартплату",
  "ипотека", "ипотеку", "ипотеки", "кредит", "кредита", "налоги", "налога",
  "штраф", "штрафы", "страховка", "страховку", "подписка", "подписку",
  "аренда", "аренду", "садик", "садике", "школа", "школу",
  "счетчик", "счетчика", "счетчики", "счетчиков",
  "кровь", "лекарство", "лекарства", "давление", "температура", "прививка",
  "прививку", "антибиотик", "витамин", "уколы", "капли", "сироп",
  // Винительный падеж: слово чаще звучит как дополнение, а не как подлежащее.
  // Без этих форм «тимпературу» не находило «температура» — разошлись хвосты.
  "температуру", "температуры", "давления", "прививки", "лекарству",
  "квартплаты", "ипотеке", "кредита", "налога", "налоги", "штрафы", "штрафа",
  "страховки", "подписки", "аренды", "поликлиники", "больницы",
  "стоматолога", "парикмахера", "маникюра", "стрижки", "планерки",
  "собрания", "бассейна", "пробежки", "зарядки", "спортзала",
  "поликлиника", "поликлинику", "поликлинике", "больница", "больницу",
  "стоматолог", "стоматологу", "парикмахер", "маникюр", "стрижка", "стрижку",
  "планерка", "планерку", "собрание", "созвон", "созвона",
  "спортзал", "бассейн", "бассейне", "пробежка", "пробежку", "зарядка",
  "сыворотка", "сыворотку", "маска", "маску", "годовщина", "годовщину",
  "днюха", "мысль", "покупки", "покупку", "хостинг",
  // глаголы и обстоятельства
  "позвонить", "перезвонить", "поставить", "поехать", "заехать", "купить", "сдать",
  "напомни", "напомнить", "завтра", "послезавтра", "сегодня", "вечером", "утром",
  "половине", "после", "через", "десять", "одиннадцать", "двенадцать",
  "тринадцать", "пятнадцать", "двадцать", "тридцать",
];

// Обычные слова, чей глухой ключ случайно совпал со словарным. Правило простое:
// сомнение — в пользу того, что человек сказал именно это слово. «Стать лучше»
// не должно превращаться в «Сдать лучше».
const STT_KEEP = new Set([
  "стать", "стал", "стала", "стали", "стать", "пора", "порой", "спать",
  "мокасин", "капать", "копать", "тапки", "папка", "палка",
]);

function sttKey(word) {
  return word
    .replace(/[ьъ]/g, "")
    // безударные гласные в речи сливаются
    .replace(/[оа]/g, "а")
    .replace(/[еияы]/g, "и")
    // звонкие оглушаются
    .replace(/б/g, "п").replace(/д/g, "т").replace(/г/g, "к")
    .replace(/з/g, "с").replace(/в/g, "ф").replace(/ж/g, "ш")
    // удвоение на слух не различить
    .replace(/(.)\1+/g, "$1");
}

const STT_FIX = (() => {
  const byKey = new Map();
  const known = new Set(STT_VOCAB);
  for (const word of STT_VOCAB) {
    if (word.length < 5) continue;
    const key = sttKey(word);
...
/**
 * Выбор формы, когда на ключ претендует несколько слов.
 *
 * «Октября» и «октябре» дают один ключ: и «я», и «е» схлопываются в «и».
 * Раньше такой ключ выключался целиком, и опечатки в месяцах не чинились вовсе.
 * Но подсказка есть в самом услышанном слове — его последняя буква. «Актябре»
 * кончается на «е», значит человек сказал предложный падеж, а не родительный.
 * Если и по последней букве однозначности нет — молчим, догадка дороже ошибки.
 */
function pickSttForm(list, word) {
  if (!list || !list.length) return null;
  if (list.length === 1) return list[0];
  const tail = word[word.length - 1];
  const sameTail = list.filter(w => w[w.length - 1] === tail);
  return sameTail.length === 1 ? sameTail[0] : null;
}

function fixSttWords(text) {
  return String(text || "").replace(/[а-яё]{5,}/giu, match => {
    const low = match.toLowerCase().replace(/ё/g, "е");
    if (STT_FIX.known.has(low) || STT_KEEP.has(low)) return match;
    return pickSttForm(STT_FIX.byKey.get(sttKey(low)), low) || match;
  });
}

function fixSttGlitches(text) {
  return fixSttWords(String(text || ""))
...
function applySynonyms(text) {
  ...
  const pairs = [
    ...
    [/натренироваться|потренироваться|потренить|потренится/giu, "тренировка"],
    [/кажд?ный|кажный/giu, "каждый"],
...
export function parse(text, ctx) {
  ...
  for (const chunk of splitChunks(clean)) {
    ...
    if (fix && fix.negated) {
      ...
      } else if (isObjectSwap(fix, head, tail, tailPlace)) {
        // «купить молоко, хотя нет, кефир» → «Купить кефир».
        // base оставляем целиком: глагол из головы всё ещё нужен для выбора полки.
        corrected = true;
        base = `${fix.head} ${fix.tail}`.trim();
        when = mergeCorrection(head, tail);
        restText = swapLastWord(head.rest == null ? fix.head : head.rest, fix.tail);
