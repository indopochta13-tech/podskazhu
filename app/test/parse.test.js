...
const cases = [
  ...
  // ─── Баг 1: день рождения ───
  // Команда «запиши», сам оборот «день рождения» и хвост «года рождения» — это
  // не имя. Имя человека приходит в родительном падеже, а в названии нужен
  // именительный. Год рождения — отдельное поле: по нему карточка считает возраст.
  {
    text: "запиши день рождения Марии 8 июня 1985 года рождения",
    test: r => {
      const i = r.drafts[0];
      return i.type === "bday" && i.title === "Мария"
        && i.date.day === 8 && i.date.month === 5 && i.birthYear === 1985;
    },
  },
  {
    text: "ДР мамы 12 марта",
    test: r => {
      const i = r.drafts[0];
      return i.type === "bday" && i.title === "Мама"
        && i.date.day === 12 && i.date.month === 2 && !i.birthYear;
    },
  },
  {
    text: "у Пети днюха 3 июня 2015",
    test: r => {
      const i = r.drafts[0];
      return i.type === "bday" && i.title === "Петя"
        && i.date.day === 3 && i.date.month === 5 && i.birthYear === 2015;
    },
  },
  {
    text: "день рождения Ивана Петровича 5 мая 1980 года",
    test: r => {
      const i = r.drafts[0];
      return i.type === "bday" && i.title === "Иван Петрович"
        && i.date.day === 5 && i.date.month === 4 && i.birthYear === 1980;
    },
  },
  {
    // Без имени сам оборот и есть название: «День рождения» лучше, чем «Без названия».
    text: "20 сентября день рождения",
    test: r => r.drafts[0].type === "bday" && /рождени/i.test(r.drafts[0].title),
  },

  // ─── Баг 2: будильник с датой ───
  // «17 часов» без предлога — это время на часах. Раньше час терялся целиком,
  // будильник уходил без времени и добирал его из умолчаний карточки.
  {
    text: "поставь будильник на завтра 17 часов",
    test: r => {
      const i = r.drafts[0];
      return i.type === "alarm" && i.date.day === 3 && i.time?.hour === 17
        && i.time?.minute === 0 && !i.repeat && !i.needsTime;
    },
  },
  {
    text: "будильник завтра 7 часов",
    test: r => {
      const i = r.drafts[0];
      return i.type === "alarm" && i.date.day === 3 && i.time?.hour === 7 && !i.repeat;
    },
  },

  // ─── Баг 3: будильник по дням недели ───
  // Перечисление дней без «по …ам» — тоже недельный повтор, а не одна дата.
  {
    text: "поставь будильники на понедельник, среда, пятница на 17 часов",
    test: r => {
      const i = r.drafts[0];
      return i.type === "alarm" && i.time?.hour === 17
        && i.repeat?.kind === "weekly" && String(i.repeat.days) === "1,3,5";
    },
  },
  {
    text: "по вторникам и четвергам в 8",
    test: r => {
      const i = r.drafts[0];
      return i.repeat?.kind === "weekly" && String(i.repeat.days) === "2,4" && i.time?.hour === 8;
    },
  },
  {
    text: "в понедельник среду и пятницу в 7 утра",
    test: r => {
      const i = r.drafts[0];
      return i.repeat?.kind === "weekly" && String(i.repeat.days) === "1,3,5"
        && i.time?.hour === 7 && !/среду|пятниц/i.test(i.title);
    },
  },
  {
    // У будильника режимов ровно четыре: однократно, ежедневно, по будням,
    // выборочно по дням. «Выходные» кладём в выборочные — суббота и воскресенье.
    text: "поставь будильник каждые выходные в 9",
    test: r => {
      const i = r.drafts[0];
      return i.type === "alarm" && i.time?.hour === 9
        && i.repeat?.kind === "weekly" && String(i.repeat.days) === "0,6";
    },
  },
  {
    text: "поставь будильник по будням в 6:30",
    test: r => {
      const i = r.drafts[0];
      return i.type === "alarm" && i.repeat?.kind === "weekdays"
        && i.time?.hour === 6 && i.time?.minute === 30;
    },
  },
  {
    // Не будильник — «выходные» остаются собственным видом повтора.
    text: "каждые выходные в 9 пробежка",
    test: r => r.drafts[0].repeat?.kind === "weekends",
  },

  // ─── Баг 4: платежи ───
  // «поставь», «платёж», «оплатить», «заплатить» — команда, а не название.
  // Название — то, за что платят; сумма выносится в отдельное поле.
  {
    text: "поставь платеж 3000 за детский сад",
    test: r => {
      const i = r.drafts[0];
      return i.type === "bills" && i.amount === 3000 && i.title === "Детский сад — 3000 ₽";
    },
  },
  {
    text: "оплатить 500 за интернет пятого",
    test: r => {
      const i = r.drafts[0];
      return i.type === "bills" && i.amount === 500 && i.date?.day === 5
        && /^Интернет/.test(i.title) && !/пятого|оплатить/i.test(i.title);
    },
  },
  {
    text: "платёж за кружок 2500 каждый месяц",
    test: r => {
      const i = r.drafts[0];
      return i.type === "bills" && i.amount === 2500
        && /^Кружок/.test(i.title) && i.repeat?.kind === "monthly";
    },
  },
  {
    text: "заплатить 1200 за телефон",
    test: r => {
      const i = r.drafts[0];
      return i.type === "bills" && i.amount === 1200 && i.title === "Телефон — 1200 ₽";
    },
  },

  // ─── Будильник: команда в названии ───
  // «разбуди», «разбуди меня», «подними» — это просьба, а не имя записи.
  {
    text: "разбуди меня в семь тридцать завтра",
    test: r => {
      const i = r.drafts[0];
      return i.type === "alarm" && i.title === "Будильник"
        && i.date.day === 3 && i.time?.hour === 7 && i.time?.minute === 30;
    },
  },
  {
    text: "разбуди в понедельник в половине седьмого",
    test: r => r.drafts[0].type === "alarm" && r.drafts[0].title === "Будильник",
  },
  {
    text: "подними меня в 6 утра",
    test: r => {
      const i = r.drafts[0];
      return i.type === "alarm" && i.title === "Будильник" && i.time?.hour === 6;
    },
  },
  {
    // Своё название у будильника остаётся: команду снимаем, смысл — нет.
    text: "поставь будильник на зарядку в 7",
    test: r => /зарядк/i.test(r.drafts[0].title),
  },

  // ─── Будильник: несколько часов в одной просьбе ───
  {
    text: "поставь два будильника на среду в 7 и в 8",
    test: r => {
      if (r.drafts.length !== 2) return false;
      const [a, b] = r.drafts;
      return a.type === "alarm" && b.type === "alarm"
        && a.time?.hour === 7 && b.time?.hour === 8
        && a.date.day === 5 && b.date.day === 5;
    },
  },

  // ─── Будильник: дата и час не спорят ───
  // «на 1 сентября» уже закрыто датой, но правило часа цеплялось за «на 1»:
  // маска проверялась только по первому символу совпадения.
  {
    text: "поставь будильник на 1 сентября в 7",
    test: r => {
      const i = r.drafts[0];
      return i.date.day === 1 && i.date.month === 8 && i.time?.hour === 7;
    },
  },
  {
    text: "встреча 5 октября в 3 часа дня",
    test: r => {
      const i = r.drafts[0];
      return i.date.day === 5 && i.date.month === 9 && i.time?.hour === 15;
    },
  },

  // ─── Будильник: дни недели через пробел и «на выходные» ───
  {
    text: "будильник по понедельникам средам в 6",
    test: r => {
      const i = r.drafts[0];
      return i.repeat?.kind === "weekly" && String(i.repeat.days) === "1,3" && i.time?.hour === 6;
    },
  },
  {
    // У будильника «на выходные» — это каждые выходные, а не ближайшая суббота.
    text: "будильник на выходные в 11 часов",
    test: r => {
      const i = r.drafts[0];
      return i.type === "alarm" && i.time?.hour === 11
        && i.repeat?.kind === "weekly" && String(i.repeat.days) === "0,6";
    },
  },
  {
    // Не будильник — «на выходных» остаётся ближайшей субботой без повтора.
    text: "на выходных помыть машину",
    test: r => r.drafts[0].date.day === 8 && !r.drafts[0].repeat,
  },

  // ─── Платежи: сумма и «за что-то» решают за словарь ───
  {
    text: "заплатить за няню 25000 в пятницу",
    test: r => {
      const i = r.drafts[0];
      return i.type === "bills" && i.amount === 25000 && i.date.day === 7 && /^Няня/.test(i.title);
    },
  },
  {
    text: "оплатить 640 рублей за домофон",
    test: r => {
      const i = r.drafts[0];
      return i.type === "bills" && i.amount === 640 && i.title === "Домофон — 640 ₽";
    },
  },
  {
    text: "перевести 3000 за репетитора в среду",
    test: r => {
      const i = r.drafts[0];
      return i.type === "bills" && i.amount === 3000 && /^Репетитор/.test(i.title);
    },
  },
  {
    // Без суммы словарь по-прежнему главный: «оплатить хостинг» — дело.
    text: "оплатить хостинг завтра",
    test: r => r.drafts[0].type === "task",
  },

  // ─── Платежи: винительный падеж в названии ───
  {
    text: "поставь платеж 15000 за ипотеку каждый месяц",
    test: r => r.drafts[0].title === "Ипотека — 15000 ₽" && r.drafts[0].repeat?.kind === "monthly",
  },
  {
    text: "оплатить страховку 18500 в октябре",
    test: r => r.drafts[0].title === "Страховка — 18500 ₽",
  },
  {
    text: "заплатить 890 за подписку",
    test: r => r.drafts[0].title === "Подписка — 890 ₽",
  },
  {
    // Два слова не трогаем: «по рисованию» — не винительный падеж названия.
    text: "оплатить кружок по рисованию 3200",
    test: r => /кружок по рисованию/i.test(r.drafts[0].title),
  },

  // ─── День рождения: падежи ───
  {
    text: "день рождения мужа 28 июля 1987 года рождения",
    test: r => r.drafts[0].title === "Муж" && r.drafts[0].birthYear === 1987,
  },
  {
    // «-ия» проверяем по словарю мужских имён: иначе «Мария» стала бы «Марий».
    text: "день рождения Дмитрия Ивановича 4 февраля 1968",
    test: r => r.drafts[0].title === "Дмитрий Иванович" && r.drafts[0].birthYear === 1968,
  },
  {
    text: "день рождения Юрия 12 апреля",
    test: r => r.drafts[0].title === "Юрий",
  },
  {
    text: "день рождения Марии 8 июня",
    test: r => r.drafts[0].title === "Мария",
  },
  {
    text: "день рождения Валерии 3 мая",
    test: r => r.drafts[0].title === "Валерия",
  },

  // ─── Несколько будильников: команда с числительным и «и в N» ───
  // Второй час — часть той же команды, в названии ему делать нечего.
  {
    text: "поставь три будильника на завтра в 6 и в 7",
    test: r => r.drafts.length === 2 && r.drafts.every(i => i.title === "Будильник")
      && r.drafts[0].time.hour === 6 && r.drafts[1].time.hour === 7,
  },
  {
    // «Пораньше» — это про час, а не имя записи.
    text: "разбуди пораньше в среду в 6",
    test: r => r.drafts[0].title === "Будильник" && r.drafts[0].date.day === 5,
  },

  // ─── Платежи: ещё один глагол и прилагательное в названии ───
  {
    text: "перечислить 5200 за газ до пятого",
    test: r => r.drafts[0].title === "Газ — 5200 ₽" && r.drafts[0].date.day === 5,
  },
  {
    // «-ого» ни у одного существительного не бывает именительным,
    // поэтому прилагательное поправить безопасно.
    text: "заплатить 14000 за зубного в пятницу",
    test: r => r.drafts[0].title === "Зубной — 14000 ₽",
  },
];
